...
const ContactCard = ({ src, delay }) => {
  ...
  if (src === "Phone") {
    icon = <MdOutlinePhoneInTalk size={50} className="text-indigo-500" />;
    content = "+91 7786123456";
  } else if (src === "Email") {
    icon = <HiOutlineMailOpen size={50} className="text-indigo-500" />;
    content = "zenmate@gmail.com";
  } else if (src === "Address") {
    icon = <FaGlobe size={50} className="text-indigo-500" />;
    content = "www.zenmate.com";
  } else {
    icon = <FaMapMarkedAlt size={50} className="text-indigo-500" />;
    content = "India";
  }
  ...
  return (
    <motion.div
      ...
      className="flex bg-indigo-500 h-[15vh] rounded-lg items-center shadow-lg"
    >
